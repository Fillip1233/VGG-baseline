...
with torch.no_grad():
    ...
    for b, data in tqdm(enumerate(dataloader)):
        try :
            os.makedirs('/mnt/cephfs/home/alvin/yingqi/STTran/video.predcls/video{}/'.format(video_figure))
        except :
            pass
        ...
        pred = model(entry)
        evaluator1.evaluate_scene_graph(gt_annotation, dict(pred),video_figure)
        evaluator2.evaluate_scene_graph(gt_annotation, dict(pred),video_figure)
        evaluator3.evaluate_scene_graph(gt_annotation, dict(pred),video_figure)
        video_figure=video_figure+1
