...
from getgt import get_gt,AverageMeter
import numpy as np
...
gpu_device = torch.device("cuda:3")
gpu_num=3
...
for epoch in tqdm(range(args.nepoch)):
    ...
    with profiler.profile(use_cuda=True) as prof:
        for b in tqdm(range(len(dataloader_train))):
            ...
            im_data = copy.deepcopy(data[0].cuda(gpu_num))
            print(im_data.shape)
            im_info = copy.deepcopy(data[1].cuda(gpu_num))
            gt_boxes = copy.deepcopy(data[2].cuda(gpu_num))
            num_boxes = copy.deepcopy(data[3].cuda(gpu_num))
            ...
            gt_annotation = [{k: v.to(gpu_device) for k, v in t.items()} for t in gt_annotation]
